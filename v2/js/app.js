...
Line 63:22 Transforming object literal extensions to the configured target environment ("es5") is not supported yet
Line 69:26 Transforming object literal extensions to the configured target environment ("es5") is not supported yet
Line 80:12 Transforming object literal extensions to the configured target environment ("es5") is not supported yet
Line 84:12 Transforming object literal extensions to the configured target environment ("es5") is not supported yet
...
class VariantSelection extends HTMLElement {
  ...
  _fetchProduct() {
    return fetch(this.getAttribute('product-url')).then(response => response.json()).then(product => {
      ...
      this._loaded = true;
       var elements = document.querySelectorAll(".product__information");
...
var selectElement = document.getElementById("variant_select");

// Initialize an array to store objects with 'value' and 'data-info' properties
var optionData = [];

// Loop through the options and collect their value and data attributes
selectElement.querySelectorAll("option").forEach(function(option) {
  var optionValue = option.value;
  var optionDataInfo = option.getAttribute("data-parent");

  optionData.push({ value: optionValue, info: optionDataInfo });
});
const priceServing = document.querySelector('.js-serving-price');
const priceSaleServing = document.querySelector('.js-price-autodeliver');
const priceVariantTotal = document.querySelector('.js-variant-total');
const priceSubsTotal = document.querySelector('.js-subs-total');
const priceBarTotal = document.querySelector('.js-price-bar-total');
const priceBarServing = document.querySelector('.js-price-bar-serving');

var currentUrl = window.location.href;
var url = new URL(currentUrl);
var variantValue = url.searchParams.get("variant");
if(variantValue != null){
  document.getElementById('variant_select').value = variantValue;
var original = document.getElementById('variant_select').options[document.getElementById('variant_select').selectedIndex].text;
var fixedPrice = document.getElementById('variant_select').options[document.getElementById('variant_select').selectedIndex].getAttribute("data-price");
var variantTitle = original.split(" / ")[1].split(" - ")[0];
var first_title = original.split(" / ")[0];

document.querySelector('.other_option input[value="' + variantTitle + '"]').checked = true;
if(first_title == "Green Apple"){
  document.querySelectorAll(".product-sell-options-wrap .option-title__label").forEach(function(element) {
    element.style.display = "none";
  });
  
  document.querySelectorAll(".product-sell-options-wrap .selling-plan-fieldset").forEach(function(element) {
      element.style.display = "none";
  });
  
  var sellingPlanPrices = document.querySelectorAll(".product-sell-options-wrap .selling-plan-price");
  // Use the last element in the NodeList
  if (sellingPlanPrices.length > 0) {
      sellingPlanPrices[sellingPlanPrices.length - 1].style.display = "none";
  }
}
setTimeout(() => {
    var price, totalPriceSale, priceBarCal; // Assuming these variables are declared somewhere in your code

    if (variantTitle.includes('20 Servings')) {
        price = fixedPrice / 20;
        priceServing.innerHTML = '(' + formatMoney(price) + ' / serving)';
        priceSaleServing.innerHTML = '(' + formatMoney(totalPriceSale / 20) + ' / serving)';
        priceBarServing.innerHTML = ' (' + formatMoney(priceBarCal / 20) + ' / serving)';
    } else if (variantTitle.includes('60 Servings')) {
        price = fixedPrice / 60;
        priceServing.innerHTML = '(' + formatMoney(price) + ' / serving)';
        priceSaleServing.innerHTML = '(' + formatMoney(totalPriceSale / 60) + ' / serving)';
        priceBarServing.innerHTML = ' (' + formatMoney(priceBarCal / 60) + ' / serving)';
    }
    if (variantTitle.includes('120 Servings')) {
        price = fixedPrice / 120;
        priceServing.innerHTML = '(' + formatMoney(price) + ' / serving)';
        priceSaleServing.innerHTML = '(' + formatMoney(totalPriceSale / 120) + ' / serving)';
        priceBarServing.innerHTML = ' (' + formatMoney(priceBarCal / 120) + ' / serving)';
    }
    if (variantTitle.includes('3 Tubes')) {
        price = fixedPrice / 15;
        priceServing.innerHTML = '(' + formatMoney(price) + ' / serving)';
        priceSaleServing.innerHTML = '(' + formatMoney(totalPriceSale / 15) + ' / serving)';
        priceBarServing.innerHTML = ' (' + formatMoney(priceBarCal / 15) + ' / serving)';
    }
    if (variantTitle.includes('6 Tubes')) {
        price = fixedPrice / 30;
        priceServing.innerHTML = '(' + formatMoney(price) + ' / serving)';
        priceSaleServing.innerHTML = '(' + formatMoney(totalPriceSale / 30) + ' / serving)';
        priceBarServing.innerHTML = ' (' + formatMoney(priceBarCal / 30) + ' / serving)';
    }
    if (variantTitle.includes('9 Tubes')) {
        price = fixedPrice / 45;
        priceServing.innerHTML = '(' + formatMoney(price) + ' / serving)';
        priceSaleServing.innerHTML = '(' + formatMoney(totalPriceSale / 45) + ' / serving)';
        priceBarServing.innerHTML = ' (' + formatMoney(priceBarCal / 45) + ' / serving)';
    }
}, 500);
...
// Log the array of objects with 'value' and 'data-info' to the console
...
function _getVariant(variants, options) {
  ...
  for (let i = 0; i < variants.length; i++) {
    variants.map(function(item, index){
        if(item.id == optionData[i+1].info){
           parnet_id = index;
         }
     });
    if(optionData[i+1].info ==''){
      const variant = variants[i];
      const isVariant = variant.options.every((option, index) => option === options[index]);
      if (isVariant) return variant; // We found the variant
    }else{
        
        return variants[parnet_id];
    }
  }
