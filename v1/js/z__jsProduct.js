...
Line 453:29 Transforming const to the configured target environment ("es5") is not supported yet
Line 1150:29 Transforming const to the configured target environment ("es5") is not supported yet
Line 1157:0 Transforming class syntax to the configured target environment ("es5") is not supported yet
Line 1158:13 Transforming object literal extensions to the configured target environment ("es5") is not supported yet
Line 1165:13 Transforming object literal extensions to the configured target environment ("es5") is not supported yet
Line 1182:4 Transforming const to the configured target environment ("es5") is not supported yet
Line 1183:4 Transforming const to the configured target environment ("es5") is not supported yet
Line 1203:19 Transforming destructuring to the configured target environment ("es5") is not supported yet
Line 1204:12 Transforming const to the configured target environment ("es5") is not supported yet
Line 1205:12 Transforming const to the configured target environment ("es5") is not supported yet
...
class Product {
 ...
 if ($productGallery.data('flickity')) {
      // Flickity is initialized, you can now call methods
      var handle =""
       $(".change_image_option").each(function(i, obj) {
         if($(this).children("input").is(':checked')){
           handle = $(this).children("input").attr("data-handle");
         }
       });
     
       $slides.each((index, slide) => {
       const $slide = $(slide);
        const dataTitle = $slide.data('title');
          if(dataTitle !== undefined){
             if(dataTitle.indexOf(handle) === -1){
              $productGallery.flickity( 'remove', slide );
            } 
          }
       ...
       $(".change_image_option").each(function(i, obj) {
        ...
        $(".product-gallery__thumbnail").each(function(i, obj) {
          if($(this).attr("data-title") != "undefined"){
            if($(this).attr("data-title").indexOf(handle) !== -1){
    ...
    $productGallery.on('change.flickity', () => {
     ...
     $(".change_image_option").click(function(e){
       if($(this).children("input").attr("data-variant-option-available") == "false"){
         e.preventDefault();
       }else{
          var handle = $(this).children("input").attr("data-handle");
       var count = 0;
       $(".other_option").addClass("add_other");
       $slides.each((index, slide) => {
       const $slide = $(slide);
        const dataTitle = $slide.data('title');
         if(dataTitle == undefined){
          ...
          if(dataTitle !== undefined){
             ...
             if(dataTitle.indexOf(handle) !== -1){
               $productGallery.flickity( 'append', slide );
               setTimeout(() => {
                $productGallery.flickity( 'select', count );
              }, 10);
            }
    ...
    if (($sliderArrows || $thumbnails) && window.isScreenSizeLarge()) {
      ...
      $thumbnails.on('click', event => {
        const visibleThumbnails = $thumbnails.filter(':visible');
        const index = $(event.currentTarget).index();
        var count = 0;
        visibleThumbnails.each((index, slide) => {
       const $slide = $(slide);
        const dataTitle = $slide.data('title');
         if(dataTitle !== undefined){
           count= count +1;
         ...
         const new_index = visibleThumbnails.index(event.currentTarget);
          if($(".other_option ").hasClass("add_other")){
             $productGallery.flickity('select', new_index - count);
          }else{
             $productGallery.flickity('select', new_index);
          }
  ...
  _switchVariant(product, variant, state) {
    ...
    if (this.priceUI) {
      const formatter = price => (
        price === 0
          ? Shopify.formatMoney(0, $('body').data('money-format'))
        ...
        product,
        {
          ...
          handler: (priceUIFragment, p, { variant: v }) => {
            ...
            if (v && v.available) {
              ...
              if (product.tags.includes('bulk') && this._section.querySelectorAll('.product-block--price-bulk').length > 0) {
                let variantTitle = v.title,
                    variantTitleDc = variantTitle.toLowerCase();
                if(variantTitleDc.includes('servings')) {
                  let fixedPrice = v.price,
                      numberServing = variantTitleDc.split('(')[1].split('servings')[0],
                      numberServingInt = parseInt(numberServing),
                      pricePerServing = fixedPrice / numberServingInt;

                  const span_1 = document.createElement('span');
                  span_1.classList.add('bulk-savings');
                  span_1.innerHTML = '(' + Shopify.formatMoney(pricePerServing, $('body').data('money-format')) + ' / serving)';
                  priceUIFragment.appendChild(span_1);

                  $('.js-price-bar-total').html(Shopify.formatMoney(fixedPrice, $('body').data('money-format')));
                  $('.js-price-bar-serving').html('(' + Shopify.formatMoney(pricePerServing, $('body').data('money-format')) + ' / serving)');
                }
              }
