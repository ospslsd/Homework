document.addEventListener('DOMContentLoaded', () => {
  i18next
    ...
    .init({
      ...
      initAdvCommOnScroll(); // Новый способ инициализации

    });

  // Функция для инициализации анимации только при появлении блока на экране
  let advCommScrollTrigger;
  function initAdvCommOnScroll() {
    if (advCommScrollTrigger) {
      advCommScrollTrigger.kill();
    }
    const advantagComm = document.querySelector('.advantag__comm');
    if (!advantagComm) return;
    advCommScrollTrigger = ScrollTrigger.create({
      trigger: advantagComm,
      start: 'top 80%',
      onEnter: () => {
        initadvComm();
      },
  ...
  function initadvComm() {
    ...
    const commTop = document.querySelector('.comm__anim-top');

    if (!advantagComm || !commMess || !commTrash || !commAnimBot || !commMessGood || !commTop) return;

    // Сброс состояния
    commMess.textContent = '';
    commMess.style.background = '';
    commMess.style.color = '';
    commMess.style.opacity = '1';
    commTrash.style.transform = '';
    commTrash.style.pointerEvents = '';
    commTrash.style.scale = '';
    commAnimBot.style.transform = 'scale(0)';
    commMessGood.textContent = '';
    commTop.style.transform = '';

    gsap.set(commTrash, { scale: 0 });
    gsap.set(commAnimBot, { scale: 0 });

    // Побуквенно вписываем текст в comm-mess
    const text = i18next.t('hate_comment');
    let charIndex = 0;
    const interval = setInterval(() => {
      commMess.textContent = text.slice(0, charIndex + 1);
      charIndex++;
      if (charIndex === text.length) {
        clearInterval(interval);

        // Меняем фон и цвет текста через 0.5 сек
        setTimeout(() => {
          commMess.style.background = 'rgba(43, 140, 141, 1)';
          commMess.style.color = 'white';

          // Показываем кнопку comm-trash через 0.5 сек
          setTimeout(() => {
            gsap.to(commTrash, {
              scale: 1,
              duration: 0.5,
              ease: 'power2.out',
              onComplete: () => {
                // Имитация клика: меняем стили и запускаем анимации через 0.5 сек
                setTimeout(() => {
                  commMess.style.transition = 'all .4s';
                  ...
                  commMess.style.background = 'rgba(150, 217, 210, 0.15)';
                  commMess.style.color = 'rgba(43, 140, 141, 1)';
                  gsap.to(commTop, { xPercent: -70 });
                  gsap.to(commMess, { opacity: 0 });
                  gsap.to(commTrash, {
                    scale: 0.7,
                    duration: 0.5,
                    ease: 'power2.out',
                    onComplete: () => {
                      // Показываем comm__anim-bot через 0.5 сек
                      setTimeout(() => {
                        gsap.to(commAnimBot, {
                          scale: 1,
                          duration: 0.5,
                          ease: 'power2.out',
                          onComplete: () => {
                            // Побуквенно пишем текст в comm-mess-good
                            const goodText = i18next.t('positive_comment');
                            let goodCharIndex = 0;
                            const goodInterval = setInterval(() => {
                              commMessGood.textContent = goodText.slice(0, goodCharIndex + 1);
                              goodCharIndex++;
                              if (goodCharIndex === goodText.length) {
  ...
  // Инициализация для всех переключателей языка
  document.querySelectorAll('.lang-switcher').forEach(langSwitcher => {
    // Находим контейнер переключателя (родитель .lang-switcher-c)
    let langSwitcherC = langSwitcher.closest('.lang-switcher-c');
    // Находим выпадающий список языков (универсально для десктопа и бургера)
    let langLow = null;
    if (langSwitcherC) {
      langLow = langSwitcherC.querySelector('.lang-switcher-low');
    }
    // Если не найден, ищем как соседний элемент после .lang-switcher (fallback)
    if (!langLow && langSwitcher.nextElementSibling && langSwitcher.nextElementSibling.classList.contains('lang-switcher-low')) {
      langLow = langSwitcher.nextElementSibling;
    }
    if (!langLow) return;
    const langOptions = langLow.querySelectorAll('.lang-other');
    const langSwitcherSp = langSwitcher.querySelector('span');
    // Открытие/закрытие списка языков
    langSwitcher.addEventListener('click', (e) => {
      e.stopPropagation();
      // Закрываем все остальные активные списки языков
      document.querySelectorAll('.lang-switcher-low.active').forEach(activeLow => {
        if (activeLow !== langLow) activeLow.classList.remove('active');
      });
      langLow.classList.toggle('active');
    });
    // Обработка выбора языка
    langOptions.forEach(option => {
      option.addEventListener('click', (e) => {
        langSwitcherC.classList.remove('active');
        const selectedLang = e.target.textContent.trim();
        const currentLang = langSwitcherSp.textContent.trim();
        langLow.classList.remove('active'); // Закрываем свитчер после выбора языка
    ...
    document.addEventListener('click', (e) => {
      if (!langLow.contains(e.target) && !langSwitcher.contains(e.target)) {
        langLow.classList.remove('active');
      }
    });
...
if (window.innerWidth < 1310) {
  function addBurger() {
    ...
    html.classList.toggle('over');

    if (burgerBg.classList.contains('active')) {
      // Добавляем обработчик клика для закрытия меню при клике вне его области
      document.addEventListener('click', closeBurgerOnClickOutside);
    } else {
      // Удаляем обработчик, если меню закрыто
  ...
  function closeBurgerOnClickOutside(event) {
    ...
    if (!burgerMenu.contains(event.target) && !event.target.closest('.header-burger')) {
...
function initializeApp() {
  ...
  if (window.innerWidth > 1320) {
    gsap.set('.header__r', { transform: 'translate(62%, 0)' });


    const heroInner = document.querySelector('.hero__inner');
    const headerR = document.querySelector('.header__r');
    if (heroInner && headerR) {
      ScrollTrigger.create({
        trigger: heroInner,
        start: 'bottom 60%',
        onEnter: () => {
          gsap.to(headerR, { transform: 'translate(0, 0)', duration: 0.6, ease: 'power2.out' });
        },
        onLeaveBack: () => {
          gsap.to(headerR, { transform: 'translate(62%, 0)', duration: 0.6, ease: 'power2.out' });
        }
      });
    }
  ...
  if (window.innerWidth < 1310) {
    document.querySelectorAll('.header__nav-c').forEach(item => {
      item.addEventListener('click', () => {
        ...
        const isActive = nextLow && nextLow.classList.contains('active');

        // Убираем active со всех элементов
        navs.forEach(nav => nav.classList.remove('active'));
        gals.forEach(gal => gal.classList.remove('active'));
        btns.forEach(btn => btn.classList.remove('active'));

        // Если элемент не был активным, добавляем active
        if (!isActive && nextLow) {
          nextLow.classList.add('active');
          galik.classList.add('active');
          btnik.classList.add('active');
...
document.addEventListener('DOMContentLoaded', () => {
  if (window.innerWidth > 1310) {
    document.querySelectorAll('.mob').forEach(element => {
      element.remove();
    });
  }
...
let progressTimer;
const progressStep = 100; // ms
const progressDuration = 8000; // ms

function resetProgressBars() {
  document.querySelectorAll('.tab-progress').forEach(bar => {
    bar.style.width = '0%';
  });
}

function startProgressBar(nextIndex) {
  resetProgressBars();
  const nextBtn = tabsButtons[nextIndex];
  const progressBar = nextBtn.querySelector('.tab-progress');
  let elapsed = 0;

  progressTimer = setInterval(() => {
    elapsed += progressStep;
    let percent = Math.min((elapsed / progressDuration) * 100, 100);
    progressBar.style.width = percent + '%';
    if (percent >= 100) {
      clearInterval(progressTimer);
      switchToNextTab();
      startProgressBar((nextIndex + 1) % tabsButtons.length);
    }
  }, progressStep);
}
...
function goTab(tabIndex) {
  // Убираем класс 'active' у всех табов и кнопок
  tabsElems.forEach(tab => tab.classList.remove('active'));
  tabsButtons.forEach(button => button.classList.remove('active'));

  // Добавляем класс 'active' к выбранному табу и кнопке
  tabsElems[tabIndex].classList.add('active');
  tabsButtons[tabIndex].classList.add('active');

  // Сбрасываем и запускаем прогресс-бар для выбранного таба
  stopAutoSwitch();
